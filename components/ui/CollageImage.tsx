...
export function CollageImage({
  ...
  const y = useTransform(scrollYProgress, [0, 1], reducedMotion || !parallax ? [0, 0] : [-30, 30]);

  return (
    <motion.div
      ref={ref}
      style={{ y: parallax ? y : 0, rotate: rotation }}
      className={cn(
        "relative overflow-hidden border transition-shadow duration-500 hover:shadow-xl",
        borderColor === "gold" ? "border-gold/50 hover:border-gold" : "border-espresso/20 hover:border-espresso/40",
        className
      )}
    >
      <Image
